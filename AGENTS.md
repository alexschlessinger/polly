...
Request flow: `main` → provider router (`llm.NewMultiPass`) → `llm.NewAgent` → agent loop (`ChatCompletionStream`, stream events parsed per-provider in `llm/adapters/`) → tool calls run in parallel via `tools.ToolRegistry` → results fed back until done or `ErrMaxIterations`.
