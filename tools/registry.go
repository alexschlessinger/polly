...
func (r *ToolRegistry) Derive(opts ...DeriveOption) *ToolRegistry {
	...
	// Derivation copies registry state and nothing else: the sandbox
	// settings travel as they are, prepared or not, and no native tools are
	// installed. The view finds native constructors through the parent.
	r.sandboxConfigMu.Lock()
	options := registryOptions{
		sandboxFactory:        r.sandboxFactory,
		baseSandboxCfg:        r.baseSandboxCfg.Merge(sandbox.Config{}),
		baseSandboxPrepared:   r.baseSandboxPrepared,
		baseSandboxPrepareErr: r.baseSandboxPrepareErr,
		unsafeNoSandbox:       r.unsafeNoSandbox,
	}
	r.sandboxConfigMu.Unlock()
	derived := newRegistry(options)
	derived.parent = r
