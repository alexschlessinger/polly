...
func TestSocketFilterProgramDecisions(t *testing.T) {
	...
	calls := []struct {
		...
		{"socketpair unix seqpacket", socketpair(unix.AF_UNIX, unix.SOCK_SEQPACKET), constant(allow)},
		{"socketpair unix seqpacket with flags", socketpair(unix.AF_UNIX, unix.SOCK_SEQPACKET|unix.SOCK_CLOEXEC|unix.SOCK_NONBLOCK), constant(allow)},
		{"socketpair unix dgram", socketpair(unix.AF_UNIX, unix.SOCK_DGRAM), constant(deny)},
		{"socketpair vsock seqpacket", socketpair(unix.AF_VSOCK, unix.SOCK_SEQPACKET), constant(deny)},
