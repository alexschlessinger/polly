...
func (r *ToolRegistry) BindExecutionContext(ec ExecutionContext, allow []string) (*ToolRegistry, []string, error) {
	...
	for _, original := range r.All() {
		...
		if name == "spawn_agent" || name == "followup_task" || name == "interrupt_agent" || name == "send_message" || name == "wait_agent" || name == "list_agents" || strings.HasPrefix(name, "swarm_") || strings.HasPrefix(name, "workflow_") {
	...
	bound.viewAllowed = func(name string) bool {
		if name == "spawn_agent" || name == "followup_task" || name == "interrupt_agent" || strings.HasPrefix(name, "workflow_") {
			return false
		}
