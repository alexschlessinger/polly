...
Model tools and JavaScript use full Git commits to identify captured code. `spawn_agent`, `polly.agent`, `polly.context`, `polly.followup`, and `swarm_publish` accept `commit` instead of `snapshot`. `polly.snapshot(context)` returns `{commit, tree, source}`; pass `.commit`, never a capture record ID. Detailed task views and JavaScript task reads expose `baseCommit` and `resultCommit`, and publication views expose `commit`. Integration candidates keep their own IDs; nested versions in inputs, conflicts, plans, and receipts expose commits without snapshot IDs.

`spawn_agent`, `polly.agent`, `polly.context`, and `polly.followup` accept full commit IDs already present in the source repository as well as retained captures. New local commits are validated against source/content policy and pinned using existing snapshot records, preserving their original SHA, tree, and ancestry. Omission still captures eligible current files, including dirty and untracked files, into a parentless commit. Commits containing private or denied paths, submodules, or content filters are rejected rather than rewritten. Branch names, abbreviated hashes, revision expressions, non-commit objects, and unknown commits are refused. Publications still require a retained capture. Explicit selection after forgetting can retain a still-local commit again under a new record; it never restores old task/submission identity. Duplicate captures of one commit are compatible baselines, but ownership, submission records, task revisions, and acceptance remain separate checks. Automatic capture ancestry is unchanged. Current public views omit commit fields when a capture is absent or forgotten, including nested integration receipts; recorded outcomes and raw historical records remain available.
