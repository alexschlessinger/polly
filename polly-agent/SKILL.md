...
cat err/<id>.log                                # why a failed agent failed
...
Without `--schema`, a `--maxtokens` cap can append a trailing
`Warning: response truncated...` line to stdout — use `--schema` for clean machine output.
