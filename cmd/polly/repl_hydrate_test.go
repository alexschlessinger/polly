...
// ID first, then by name, denied calls show as denied whatever their row
// held, and rows the record never mentions keep their place at the end.
func TestHistoryHydratorAppliesDurableToolOrder(t *testing.T) {
	m := newReplModel()
	h := historyHydrator{m: m}
	h.toolRows = []toolDisclosureRow{
		{callID: "b", label: "read", line: "read-line", settled: true},
		{callID: "", label: "bash", line: "bash-line", settled: true},
		{callID: "z", label: "extra", line: "extra-line", settled: true},
	}
	h.flushTools()
	...
	got := h.tools.rows
	wantLabels := []string{"bash", "read", "write", "extra"}
	wantLines := []string{"bash-line", "read-line", toolDeniedLine("write"), "extra-line"}
