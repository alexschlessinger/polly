...
//
// Only r.model is written here. Every other model that holds resolved cells
// (the tabs in the background, the inspector's open view, the cached child and
// main projections) is painted under its own lock and must not have its caches
// written from this one: a background tab's turn is appending to its model at
// the same time. Those need no write, because each re-parses on its own next
// paint:
//
