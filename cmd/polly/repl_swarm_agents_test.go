...
func TestSwarmIterationPauseRendersReasonAndResumesThroughCommand(t *testing.T) {
	...
	runtime, err := swarm.New(swarm.Config{Store: store, Parent: parent, Registry: registry, OpenTools: tools.NativeOpenTools(registry), Client: model, Root: t.TempDir(), Directory: t.TempDir(), Agent: llm.AgentConfig{MaxIterations: 1}})
	if err != nil {
		t.Fatal(err)
	}
	defer runtime.Close()
