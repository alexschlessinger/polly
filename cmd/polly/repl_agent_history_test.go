...
func TestAgentHistorySelectionAndDeferral(t *testing.T) {
	...
	report, err := runtime.RunWorkflow(ctx, `polly.defineWorkflow({name:"audit fixture",inputSchema:polly.schema.object({}),async run(){await polly.agent({task:"inspect",readOnly:true});polly.fail("incomplete")}})`, map[string]any{})
	if err == nil {
...
func TestHistoryOrdersAttemptsWithoutStartingRuntimes(t *testing.T) {
	...
	for _, name := range []string{"older", "newer"} {
		report, err := root.state.swarm.RunWorkflow(ctx, `polly.defineWorkflow({name:"`+name+`",inputSchema:polly.schema.object({}),async run(){const a=await polly.agent({task:"read",readOnly:true});const t=await polly.tasks.read(a.task);await polly.tasks.review({task:t.id,revision:t.revision,accept:true});return null}})`, map[string]any{})
...
func TestAgentHistoryForExternallyLeasedRoot(t *testing.T) {
	...
	root := r.visibleTab()
	ctx := context.Background()
	report, err := root.state.swarm.RunWorkflow(ctx, `polly.defineWorkflow({name:"saved audit",inputSchema:polly.schema.object({}),async run(){await polly.agent({task:"read",readOnly:true});polly.fail("incomplete")}})`, map[string]any{})
	if err == nil || report == nil {
		t.Fatal("fixture did not fail")
	}
