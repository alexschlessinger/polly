...
const {obj, str, arr, bool, enum: senum, keyed} = polly.schema;
const nonblank = str({minLength: 1, pattern: "\\S"});
const claim = obj({id: nonblank, claim: nonblank, evidence: nonblank});
const enumeration = obj({claims: arr(claim, {minItems: 1})});
const verdict = obj({
  verdict: senum("holds", "drifted", "stale", "unverifiable"),
  evidence: nonblank,
  suggestedChange: str(),
}, {allOf: [{
...
async function auditArea(input, area) {
  ...
  const found = await polly.research(label + " claim enumerator",
    "Read the documentation file named in the input. Extract every claim a reader could act on: tool names, commands, flags, config fields, defaults, environment variables, and platform behavior. Give each claim a stable kebab-case id and document path:line evidence. Omit vague statements that cannot be checked against the code. An empty enumeration is incomplete, not a clean audit. You cannot edit anything.", {
    source: input.source,
  ...
  async function verify(previous, commit) {
    const suffix = commit ? " This is a repaired candidate: use original claim IDs to track each issue, but inspect the revised documentation. Mark holds when the current documentation correctly describes the code or removes an obsolete claim; do not require the old incorrect wording to remain true." : "";
    const review = await polly.research(label + " verifier",
      "Independently verify EVERY claim in the input against the documentation, actual code, tests, and repository instructions in your assigned copy. Trace each command, flag, and default to its implementation. Treat prior verdicts and editor reports as claims, not facts. A claim is drifted when the documentation disagrees with the code, stale when it describes removed behavior, and unverifiable when it cannot be established. Cite documentation and implementation path:line references as evidence, describe what was checked for unverifiable claims, and give an actionable suggestedChange for drifted/stale verdicts. You cannot edit or commit." + suffix, {
      ...(commit ? {commit} : {source: input.source}),
      input: {doc: area.path, claims, previous: previous || []},
      schema: keyed(ids(claims), verdict),
    });
  ...
  if (drifted.length && input.repair !== false) {
    ...
    editor = await polly.editor(input.source, label + " doc editor",
      "Repair documentation drift in your assigned isolated copy. Read the repository instructions, then update only the documentation file named in the input so every drifted claim matches the code. Preserve unrelated prose and files. Do not commit or push. Report every claim id, including anything skipped.", {
      input: {doc: area.path, drifted},
      schema: keyed(ids(drifted), fix),
    });
...
polly.workflow("doc-drift-audit", obj({
