...
const {obj, str, bool} = polly.schema;
const quote = text => "'" + text.replace(/'/g, "'\\''") + "'";

polly.workflow("integration-evidence-exercise", obj({path: str({minLength: 1})}), async (input) => {
  ...
  async function edit(label, commit, content) {
    const result = await polly.agent(label, "Integration evidence exercise: " + label + ". Set only " + JSON.stringify(input.path) +
      ...
      commit, tools: ["read_file", "write_file"],
    });
    contexts.push(result.context);
    const {id, revision, resultCommit} = await polly.tasks.get(result.task);
    return {task: id, revision, commit: resultCommit, session: result.session};
  }
  ...
  const review = await polly.research("exercise reviewer", "Review the integration evidence exercise. Read the fixture and confirm its exact contents, with both alpha and beta and no conflict markers. Return approved only when it matches the supplied expected contents. Do not edit.", {
    commit: validatedCommit, tools: ["read_file"],
    input: {path: input.path, expected, commit: validatedCommit},
    schema: obj({approved: bool(), feedback: str()}),
  });
