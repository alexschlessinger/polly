...
func TestWorkflowValidationShellExitSemantics(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"sequence final status", "false; printf 'later success\\n'", true, false, 0},
		{"pipeline final status", "false | cat; printf 'later success\\n'", true, false, 0},
		{"failed producer before tail", "(printf 'FAIL\\n'; exit 3) | tail -n 1", true, true, 3},
		{"collected failed producer before tail", "(printf 'FAIL\\n'; exit 3) | tail -n 1", false, false, 3},
		{"strict failed sequence stops", "set -e -o pipefail; false; printf 'later success\\n'", true, true, 1},
		{"strict failed pipeline stops", "set -e -o pipefail; false | cat; printf 'later success\\n'", true, true, 1},
		{"collected strict sequence", "set -e -o pipefail; false; printf 'later success\\n'", false, false, 1},
		{"collected strict pipeline", "set -e -o pipefail; false | cat; printf 'later success\\n'", false, false, 1},
		{"early pipeline reader fails", "yes | head -n 1", true, true, 141},
		{"early pipeline reader opt out", "set +o pipefail; yes | head -n 1", true, false, 0},
		{"grep substitution no matches", "count=$(grep -c absent features.txt); printf '%s' \"$count\"", true, false, 0},
