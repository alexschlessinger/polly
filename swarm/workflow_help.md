...
| `publications({kind?})` | What this run's agents can read, oldest first: the run's findings and every host fact published in the swarm (`kind:"host"` or `"finding"` keeps one), the latest of each supersede chain, each with `id`, `author`, `kind`, `text`, `sources`, `readBy`, and `posted`. Pass host facts to agents as `hostNotes`. |
...
Scoped `work` exposes agent, research, editor, followup, integrate, context, tool, exec, snapshot, release, and log. followup/integrate use explicit arguments rather than scope defaults. schema, keyed, parallel, tasks, integration, publications, fail, workflow, and defineWorkflow remain on polly.
...
For a new member, source selects snapshot input and context selects the workspace to copy; commit selects an immutable Git commit from that repository or a retained capture. Agents run in their assigned directories. Continue an existing active obligation with `agent({session: prior.session, task: "follow-up brief"})`; it preserves the saved conversation and authority. The same call retries a member of this workflow whose last execution failed: the conversation stays, the result schema is inherited unless supplied, and the new execution starts with fresh corrections and spends a start; a paused member and another workflow's member are refused, and the refusal carries no session, so keep the id from the failure. Do not override source, commit, context, model, or tools on continuation. Use followup for new work after a task is complete. To run a task created beforehand, use agent({taskID: task.id, label:"Worker", task:task.description, readOnly:true}) with a compatible role, after its dependencies are done. The scheduler assigns it; no claim or submit call is needed. Successful final results are captured automatically; swarm_block leaves the task unresolved. JavaScript cannot set maxIterations or grant budgets.
...
Reaching for a member a namespace does not have raises `polly has no member 'agnet'; it has agent, context, ...`, naming the namespace and listing every member it holds; `polly.tasks`, `polly.integration`, `polly.schema` and a `polly.scope` work object each report themselves the same way. Read the list rather than guessing a second name. Test for a member without raising with `"agent" in polly`.
...
The parent model can use `followup_task({target,message,refresh:true})` after a worker's assignment is done and its execution is idle. Default follow-ups preserve its previous code; refresh captures current parent files and replaces only an eligible workspace, preserving conversation and authority. The worker's scratch is carried into the new workspace; non-Git research retains its saved live root. Paused or pending work, live workflow reservations, uncertain integrations, retained workspaces and extra worker edits are refused. No acceptance or budget grant is implied. `send_message` changes information only; a new worker provides independent review.
