...
| `exec(command, {context, check?})` | Runs `bash -c` under context policy; returns a tool result plus `exitCode`. `check` defaults to true and rejects a nonzero final shell exit status. Enable strict shell options explicitly when required. |
...
Run each required check in a separate awaited exec call. Enable `set -e -o pipefail` when every command and pipeline stage must succeed. Checking sees the final shell status only, and conditional-list exceptions remain: `false && printf unreachable; printf later` succeeds. Propagate required failures explicitly (`required_check || exit $?`) in compound scripts. Handle expected failures with `if`; account for intentional early-reader termination such as `yes | head` when enabling `pipefail`. External shell tools and separately launched scripts retain their own shell options. Use `go test -count=1` for mutation tests to avoid cached results.
